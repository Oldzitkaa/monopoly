<?php include_once './database_connect.php';?>
<!DOCTYPE html>
<html lang="pl">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Potęga Smaku</title>
    <link rel="stylesheet" href="./css/styl.css">
</head>
<body>
<!-- div pierwszy -->
    <div class="logo_div step1">
        <p class="monopoly size1">MONOPOLY</p>
        <img src="./zdj/logo.png" alt="Potega Smakow" class="logo_zdj">
        <button class="next nextright" onclick="one()"><p class="nextstep size3">	&larr;</p></button>
      </div>
      
<!-- div drugi -->
      <div class="logo_div step2" >
        <p class="monopoly size1">Podaj liczbe <br> graczy</p><br>
        <p id="playerCount" class="size2">2</p>
        <form action="" method="post">
          <label for="quantity">
              <input type="range" class=" rangeplayer" name="quantity" id="quantity" min="2" max="4" value="2">
          </label>
        </form>
        <button class="next nextleft" onclick="two()"><p class="nextstep size3">	&rarr;</p></button>
      </div>
      <!-- php dodawanie gry i startowanie -->
      <?php 
        $sql1 = 'INSERT INTO `games` (`current_turn`, `status`,	`created_at`) VALUES (0, "active", ?)';
          $query1 = $mysqli -> prepare($sql1);
          $c_d = date("Y-m-d");
          $query1->bind_param('s', $c_d);
          $query1->execute();
      ?>
<!-- div trzeci -->
      <div class="logo_div step3" >
        <p class="monopoly size1">Podaj niki <br> graczy</p><br>
        <!-- <select name="" id="">
          <option value=""><img src="./zdj/postacie/" alt=""></option>
        </select> -->
        
        <button class="next nextleft" onclick="two()"><p class="nextstep size3">	&rarr;</p></button>
      </div>
      <div class="logo_div characters">
      <?php
            $sql2 = 'SELECT name, plik FROM characters';
            $query2 = $mysqli -> prepare($sql2);
            $query2->execute();
            $result2 = $query2->get_result();
            while ($row = $result2->fetch_object()) {
                echo "<div>";
                echo "<img src ='" . $row->plik . "'>";
                echo "<p>" . $row->name . "</p></div>";
            }
        ?>
      </div>
      
<!-- tlo -->
      <img src="./zdj/tlo3.png" alt="" class="backg">
      
      <script src="./js/script_main.js"></script>
</body>
</html>
<!-- vw mixin
 clamp css
 bem css
  -->

  <!-- $sql = 'SELECT nazwa_przestepstwa, id FROM przestepstwa;';
                                $query = $db->prepare($sql);
                                $query->execute();
                                $result = $query->get_result();
                                while ($rob = $result->fetch_object()) {
                                    echo "<option value='$rob->id'>$rob->nazwa_przestepstwa</option>";
                                } -->